...
# muestra_bootstrap <- simular_modelo(length(x), 0 ,sqrt(est_mle))
...
x_axis <- seq(1, 1000, 1)
plot(x_axis, dinv_gamma(x_axis, alpha=aplha, beta=beta), type="l")
```
...
**Observamos que las distribuciones son bastante parecidas, aunque la distribución posterior muestra una ligera inclinación hacia la derecha**
...
ee <- reps_boot %>% summarise(ee_boot = sd(estimador_boot)) %>% pull(ee_boot)
cat("Error estándar bootstrap: ",ee)
ggplot(reps_boot, aes(x = estimador_boot)) +
  geom_histogram(title = "Replicaciones Bootstrap", fill="gray") +facet_wrap(~parametro)
```
...
posterior <- simulacion_posterior(x,alpha, beta, 5000)
intervalo_p <- quantile(posterior, c(0.025, 0.975))
print(intervalo_p)
#plot
ggplot(data.frame(posterior), aes(x = posterior)) +
  geom_histogram(fill="lightblue")
